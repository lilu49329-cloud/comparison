...
function updateRotateHint() {
    ...
    if (overlayTurnedOn) {
        ...
        const tryPlayVoiceRotate = () => {
            ...
            // Ưu tiên scene nào đã có asset voice_rotate
            let scene =
                scenes.find((s) => {
                    const cacheAny = s.cache as any;
                    const hasInCache =
                        !!cacheAny?.audio?.exists?.('voice_rotate');
                    const hasSound = !!s.sound.get('voice_rotate');
                    return hasInCache || hasSound;
                }) ?? scenes[0];

            if (!scene || !scene.sound) return;

            const cacheAny = scene.cache as any;
